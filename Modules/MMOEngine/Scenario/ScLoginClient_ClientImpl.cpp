                                         ...
                                         void* data, int size, unsigned char subNet)
{
  ...
  TBreakPacket bpLP;// контейнер дл€ всего пакета
  if(Context()->GetMS()->GetUseCryptTCP())
  {
    // если данные шифруютс€, то формировать так:
...
void TScLoginClient_ClientImpl::Disconnect()
{
  ...
  // дл€ Slave отдать свой ID, он по нему нас зарегистрирует  
