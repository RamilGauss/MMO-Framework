...
*/
// File has been generated by ReflectionCodeGenerator version 1.18 build 46, at 2020_10_18 14:38:07.357
...
void TBinaryMarshaller::Unpack( char* packedData, int size )
{
    ...
    switch( type )
    {
        ...
        break;
        case e_TSimpleComponent:
        {
            auto p = Allocate<TSimpleComponent>();
            _Deserialize( p );
            TUnpackHandler::Handle( p );
        }
        break;
...
{
    _Serialize( (nsECSFramework::IComponent*)p );
...
{
    _Deserialize( (nsECSFramework::IComponent*)p );
...
//---------------------------------------------------------------------------------------
void TBinaryMarshaller::_Serialize( TSimpleComponent* p )
{
    _Serialize( (nsECSFramework::IComponent*)p );
    mPushMaster.PushNum( p->ll );
    mPushMaster.PushNum( p->ull );
    mPushMaster.PushNum( p->in64 );
    mPushMaster.PushNum( p->uin64 );
    mPushMaster.PushStr( p->name );
}
//---------------------------------------------------------------------------------------
void TBinaryMarshaller::_Deserialize( TSimpleComponent* p )
{
    _Deserialize( (nsECSFramework::IComponent*)p );
    mPopMaster.PopNum( p->ll );
    mPopMaster.PopNum( p->ull );
    mPopMaster.PopNum( p->in64 );
    mPopMaster.PopNum( p->uin64 );
    mPopMaster.PopStr( p->name );
}
//---------------------------------------------------------------------------------------
TBinaryMarshaller::TypeID TBinaryMarshaller::_GetTypeID( TSimpleComponent* p )
{
    return e_TSimpleComponent;
}
//---------------------------------------------------------------------------------------
void TBinaryMarshaller::_Deallocate( TSimpleComponent* p )
{
}
//---------------------------------------------------------------------------------------
void TBinaryMarshaller::_Serialize( TTestClass* p )
{
    ...
    auto unknownType_p = &(p->unknownType);
    PushIsNotNullptr( unknownType_p );
    if ( unknownType_p )
        _Serialize( unknownType_p );
}
//---------------------------------------------------------------------------------------
