...
// File has been generated at 2021_07_21 08:19:04.840
...
void TShareDevJsonSerializer::Init()
{
    ...
    TypeFunc _TProjectConfigTypeFunc;
    _TProjectConfigTypeFunc.serializeFunc = [] (void* p, std::string& str) {
    Serialize<TProjectConfig>((TProjectConfig*) p, str);
    };
    _TProjectConfigTypeFunc.deserializeFunc = [] (void* p, const std::string& str, std::string& err) {
        return Deserialize<TProjectConfig>((TProjectConfig*) p, str, err);
    };
    
    auto rtti__TProjectConfigTypeFunc = globalTypeIdentifier->type<TProjectConfig>();
    
    m.insert({ rtti__TProjectConfigTypeFunc, _TProjectConfigTypeFunc });
    
    TypeFunc _TProjectInfoTypeFunc;
    _TProjectInfoTypeFunc.serializeFunc = [] (void* p, std::string& str) {
    Serialize<TProjectInfo>((TProjectInfo*) p, str);
    };
    _TProjectInfoTypeFunc.deserializeFunc = [] (void* p, const std::string& str, std::string& err) {
        return Deserialize<TProjectInfo>((TProjectInfo*) p, str, err);
    };
    
    auto rtti__TProjectInfoTypeFunc = globalTypeIdentifier->type<TProjectInfo>();
    
    m.insert({ rtti__TProjectInfoTypeFunc, _TProjectInfoTypeFunc });
...
//---------------------------------------------------------------------------------------
void TShareDevJsonSerializer::_Serialize(TConveyerConfig* p, Jobj& obj)
{
...
//---------------------------------------------------------------------------------------
void TShareDevJsonSerializer::_Deserialize(TConveyerConfig* p, const Jobj& obj)
{
...
//---------------------------------------------------------------------------------------
void TShareDevJsonSerializer::_Serialize(TProjectConfig* p, Jobj& obj)
{
    auto info_o = PUM::AddObject(obj, "info");
    _Serialize(&(p->info), info_o);
    PUM::Push(obj, "binary", p->binary);
    PUM::Push(obj, "conveyorFile", p->conveyorFile);
    ...
    PUM::Value args_a0(rapidjson::kArrayType);
    for(size_t args_i0 = 0 ; args_i0 < p->args.size() ; args_i0++) {
        auto& args_e0 = p->args[args_i0];
        PUM::PushBack(args_a0, args_e0);
    }
    PUM::Push(obj, "args", args_a0);
}
//---------------------------------------------------------------------------------------
void TShareDevJsonSerializer::_Deserialize(TProjectConfig* p, const Jobj& obj)
{
    auto info_o0 = POM::FindObject(obj, "info");
    _Deserialize(&(p->info), info_o0);
    POM::PopStr(obj, "binary", p->binary);
    POM::PopStr(obj, "conveyorFile", p->conveyorFile);
    ...
    if (POM::IsArray(obj, "args")) {
        auto args_a0 = POM::FindArray(obj, "args");
        for(auto& args_e0 : args_a0) {
            p->args.push_back(args_e0.GetString());
        }
    } else {
        std::string args_t0;
        POM::PopStr(obj, "args", args_t0);
        p->args.push_back(args_t0);
    }
}
//---------------------------------------------------------------------------------------
void TShareDevJsonSerializer::_Serialize(TProjectInfo* p, Jobj& obj)
{
...
//---------------------------------------------------------------------------------------
void TShareDevJsonSerializer::_Deserialize(TProjectInfo* p, const Jobj& obj)
{
