...
TDockingStuffWindow::TDockingStuffWindow(std::string name)
{
    ...
    mPopupNodes[0].mOnLeftClickCB.Register(this, [&](nsImGuiWidgets::TNode* pNode)
    {
        auto logLabelText = mTextEdit.GetText();
        ImGui::SetClipboardText(logLabelText.c_str());
    });

    mPopupNodes[1].mOnLeftClickCB.Register(this, [&](nsImGuiWidgets::TNode* pNode)
    {
        auto logLabelText = ImGui::GetClipboardText();
        mTextEdit.SetText(logLabelText);
    });
    mPopupNodes[2].mOnLeftClickCB.Register(this, [&](nsImGuiWidgets::TNode* pNode)
    {
        auto logLabelText = mTextEdit.GetText();
