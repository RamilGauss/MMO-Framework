...
namespace nsContainerCodeGenerator::nsAggregator::nsImGuiWidgets
{
    void TGenerateAggregatorHeaderSystem::Execute()
    {
        ...
        generatedFile.absPath = nsBase::TPathOperations::CalculatePathBy(configComponent->value.aggregator.targetDirectory,
            configComponent->value.aggregator.imGuiWidgetsImpl.impl.fileName + ".h");

        nsBase::TTextGenerator txtGen(generatedFile.content);

        txtGen.AddPragmaOnce();
        txtGen.AddEmpty();
        txtGen.AddInclude(configComponent->value.aggregator.imGuiWidgetsImpl.parent.fileName + ".h");
        txtGen.AddEmpty();
        ...
        auto type = fmt::format("{} {}", configComponent->value.projectConfig.exportDeclaration,
            configComponent->value.aggregator.imGuiWidgetsImpl.impl.typeName);

        auto parent = fmt::format("public {}::{}",
            configComponent->value.coreConfig.nameSpace, configComponent->value.aggregator.imGuiWidgetsImpl.parent.typeName);

        txtGen.AddStruct(type, { parent });
        txtGen.AddLeft();
        txtGen.IncrementTabs();

        txtGen.AddCtorDecl(configComponent->value.aggregator.imGuiWidgetsImpl.impl.typeName);
        txtGen.AddVirtualDtorDecl(configComponent->value.aggregator.imGuiWidgetsImpl.impl.typeName);
