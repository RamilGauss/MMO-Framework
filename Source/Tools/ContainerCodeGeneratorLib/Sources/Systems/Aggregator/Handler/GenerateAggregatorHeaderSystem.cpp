...
namespace nsContainerCodeGenerator::nsAggregator::nsHandler
{
    void TGenerateAggregatorHeaderSystem::Execute()
    {
        ...
        txtGen.AddEmpty();
        txtGen.AddInclude(configComponent->value.aggregator.handlerImpl.parent.fileName + ".h");
        txtGen.AddEmpty();
        ...
        auto type = fmt::format("{} {}", configComponent->value.projectConfig.exportDeclaration,
            configComponent->value.aggregator.handlerImpl.impl.typeName);

        auto parent = fmt::format("public {}::{}",
            configComponent->value.coreConfig.nameSpace, configComponent->value.aggregator.handlerImpl.parent.typeName);

        txtGen.AddStruct(type, { parent });
        txtGen.AddLeft();
        txtGen.IncrementTabs();

        txtGen.AddCtorDecl(configComponent->value.aggregator.handlerImpl.impl.typeName);
        txtGen.AddVirtualDtorDecl(configComponent->value.aggregator.handlerImpl.impl.typeName);
