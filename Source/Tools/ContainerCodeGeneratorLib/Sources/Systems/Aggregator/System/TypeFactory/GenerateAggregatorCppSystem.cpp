...
namespace nsContainerCodeGenerator::nsAggregator::nsSystem::nsTypeFactory
{
    void TGenerateAggregatorCppSystem::Execute()
    {
        ...
        pathRelToProjectSources /= configComponent->value.projectConfig.systemConfig.typeFactory.fileName + ".h";
        ...
        txtGen.AddFormatLine("auto has = {}::Has(rtti);",
            configComponent->value.projectConfig.systemConfig.typeFactory.typeName);

        txtGen.AddLine("if (has) {");
        txtGen.IncrementTabs();
        auto newRet = fmt::format("{}::New(rtti)", configComponent->value.projectConfig.systemConfig.typeFactory.typeName);
        ...
        txtGen.AddFormatLine("{}::Delete(p, rtti);", configComponent->value.projectConfig.systemConfig.typeFactory.typeName);
