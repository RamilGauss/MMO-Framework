...
namespace nsContainerCodeGenerator::nsAggregator::nsSystem
{
    void TGenerateAggregatorHeaderSystem::Execute()
    {
        ...
        generatedFile.absPath = nsBase::TPathOperations::CalculatePathBy(configComponent->value.aggregator.targetDirectory,
            configComponent->value.aggregator.systemImpl.impl.fileName);

        nsBase::TTextGenerator txtGen(lines);

        inja::json data;

        data["PARENT_FILE_NAME"] = configComponent->value.aggregator.systemImpl.parent.fileName;
        ...
        data["PARENT_TYPE_NAME"] = configComponent->value.aggregator.systemImpl.parent.typeName;
