...
void TBuilder_Model_Logic::SetupCollectionNode()
{
    ...
    for ( auto& vtPart : mModelItem->mMapNamePart ) {
        std::string namePart = vtPart.first;
        for ( auto& variant : vtPart.second.vecVariant ) {
...
void TBuilder_Model_Logic::LinkHierarchy()
{
    ...
    for ( auto& vtLocation : mModelItem->mMapNameBranchLocation ) {
        ...
        for ( TModelItem::TLink& linkModelItem : vtLocation.second.listLink ) {
    ...
    TNodeLocation_Model* pNodeLocation)
{
    ...
    for ( auto& vtEJ : pModelItem->mMapExternalJoint ) {
    ...
    TNodeLocation_Model* pNodeLocation)
{
    ...
    for ( auto& vtJoint : pShapeItem->mMapNameJoint ) {
...
void TBuilder_Model_Logic::DefineLocalLocationJoint()
{
    ...
    for ( int iPart = 0; iPart < cntPart; iPart++ ) {
        ...
        for ( auto& vtJoint : pNodeLocation->mMapNameJoint ) {
