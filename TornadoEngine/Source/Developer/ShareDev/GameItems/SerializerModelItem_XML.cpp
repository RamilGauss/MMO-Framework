...
void TSerializerModelItem_XML::SaveHierarchy()
{
  ...
  {
    for( auto& bit : mModel->mMapNameBranchLocation )
...
void TSerializerModelItem_XML::SaveCollection()
{
  ...
  {
    for( auto& bit : mModel->mMapNamePart )
    {
      TAttrInfo attr;
      attr.Name = sName;
...
void TSerializerModelItem_XML::SaveExternalJoining()
{
  ...
  {
    int iJoint = 0;
    for( auto& extJoint : mModel->mMapExternalJoint )
    {
      TAttrInfo attr[3];
...
void TSerializerModelItem_XML::SaveLocation( TModelItem::TLocation& location )
{
  if( mXML->AddSectionAndEnter( sLocation ) )
  {
    ...
    for( auto& link : location.listLink )
      SaveLink( link );
...
void TSerializerModelItem_XML::SaveLink( TModelItem::TLink& link )
{
  if( mXML->AddSectionAndEnter( sLink ) )
  {
    ...
    {
      for( auto& bit : mMapNameValue_Constraint )
      {
        std::string key, value;
        key = bit.first;
        value = bit.second;
...
{
  for( auto& variant : part.vecVariant )
  {
    TAttrInfo attr[2];
    attr[0].Name = sName;
