...
using namespace nsHeightMapFromOgre;

THeightMapFromOgre::THeightMapFromOgre()
{
...
//---------------------------------------------------------------------
void THeightMapFromOgre::Done()
{
	delete mDataStream.get();
	mDataStream.setNull();
}
//---------------------------------------------------------------------
bool THeightMapFromOgre::Setup(std::string& filename, TDataOut& data)
{
	mFileName = filename;
...
//---------------------------------------------------------------------
bool THeightMapFromOgre::Prepare()
{
...
//---------------------------------------------------------------------
void THeightMapFromOgre::readLodData(unsigned short lowerLodBound, unsigned short higherLodBound)
{
...
//---------------------------------------------------------------------
void THeightMapFromOgre::fillBufferAtLod(unsigned int lodLevel, const float* data, unsigned int dataSize )
{
...
//---------------------------------------------------------------------
unsigned int THeightMapFromOgre::TTerrainProperty::getGeoDataSizeAtLod(unsigned short lodLevel) const
{
...
//---------------------------------------------------------------------
unsigned short THeightMapFromOgre::TTerrainProperty::getResolutionAtLod(unsigned short lodLevel) const
{
	return ((mSize - 1) >> lodLevel) + 1;
}
//---------------------------------------------------------------------
int THeightMapFromOgre::TTerrainProperty::getPositiveLodLevel( int lodLevel ) const
{
	return (lodLevel>=0) ? lodLevel : mNumLodLevels+lodLevel;
}
//---------------------------------------------------------------------
THeightMapFromOgre::TTerrainProperty::TTerrainProperty()
{
	mAlign     		= 0;
	mSize      		= 0;	mWorldSize 		= 0;	mMaxBatchSize = 0;	mMinBatchSize = 0;	mPos          = Ogre::Vector3(0,0,0);
}
//---------------------------------------------------------------------
void THeightMapFromOgre::TTerrainProperty::determineLodLevels()
{
