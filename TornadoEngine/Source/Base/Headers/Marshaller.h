...
#pragma pack(pop)
  ...
  typedef std::unordered_map<std::string, TypeID> TMapStrShort;
  typedef TMapStrShort::value_type TMapStrShortVT;

  TMapStrShort mNameIDMap;
...
public:
  ...
  void SetLimitForCompression( int binarySize ){ mSetLimitForCompression = binarySize; }
...
{
  mHeader.type = mNameIDMap[typeid(Type).name()];
...
void TMarshaller<TSerializerClass>::Add( TypeID id, AllocateFunc allocatorFunc )
{
  ...
  mNameIDMap.insert( TMapStrShortVT( typeName, id ) );
