...
//}
////---------------------------------------------------------------------------------------
//void TJsonSerializer::_Deserialize( TFloat3* p, const Jobj& jobj )
//{
//    POM::PopNum( jobj, "x", p->x );
//    POM::PopNum( jobj, "y", p->y );
//    POM::PopNum( jobj, "z", p->z );
//}
////---------------------------------------------------------------------------------------
...
/*void TJsonSerializer::_Serialize( nsBS::TTestStruct* p, Jobj& obj )
{
    ...
    Jobj baseStruct_obj = PUM::AddObject(obj, "baseStruct");
...
void TJsonSerializer::_Serialize(nsReflectionCodeGenerator::TBinaryMarshallerGeneratorConfig* p, Jobj& obj)
{
    ...
    Jobj unpackHandler_obj = PUM::AddObject(obj, "unpackHandler");
...
{
    _Deserialize((nsReflectionCodeGenerator::TClassDesc*)p, obj);
    auto unpackHandler_json = POM::FindObject(obj, "unpackHandler");
...
{
    Jobj targetForParsing_obj = PUM::AddObject(obj, "targetForParsing");
    _Serialize(&(p->targetForParsing), targetForParsing_obj);

    Jobj filter_obj = PUM::AddObject(obj, "filter");
    _Serialize(&(p->filter), filter_obj);

    Jobj targetForCodeGeneration_obj = PUM::AddObject(obj, "targetForCodeGeneration");
...
{
    auto targetForParsing_json = POM::FindObject(obj, "targetForParsing");
    _Deserialize(&(p->targetForParsing), targetForParsing_json);
    auto filter_json = POM::FindObject(obj, "filter");
    _Deserialize(&(p->filter), filter_json);
    auto targetForCodeGeneration_json = POM::FindObject(obj, "targetForCodeGeneration");
...
void TJsonSerializer::_Deserialize(nsReflectionCodeGenerator::TFilter* p, const Jobj& obj)
{
    ...
    auto extensions_src0 = POM::FindArray(obj, "extensions");
...
{
    Jobj jsonSerializer_obj = PUM::AddObject(obj, "jsonSerializer");
    ...
    Jobj binaryMarshaller_obj = PUM::AddObject(obj, "binaryMarshaller");
    ...
    Jobj sql_obj = PUM::AddObject(obj, "sql");
...
{
    auto& jsonSerializer_json = POM::Find(obj, "jsonSerializer");
    if ( jsonSerializer_json.IsNull() == false ) {
        if ( p->jsonSerializer.get() == nullptr )
            p->jsonSerializer.reset(new nsReflectionCodeGenerator::TJsonSerializerGeneratorConfig());
        _Deserialize(p->jsonSerializer.get(), jsonSerializer_json.GetObject());
    }
    auto& binaryMarshaller_json = POM::Find(obj, "binaryMarshaller");
    if ( binaryMarshaller_json.IsNull() == false ) {
        if ( p->binaryMarshaller.get() == nullptr )
            p->binaryMarshaller.reset(new nsReflectionCodeGenerator::TBinaryMarshallerGeneratorConfig());
        _Deserialize(p->binaryMarshaller.get(), binaryMarshaller_json.GetObject());
    }
    auto& sql_json = POM::Find(obj, "sql");
    if ( sql_json.IsNull() == false ) {
        if ( p->sql.get() == nullptr )
            p->sql.reset(new nsReflectionCodeGenerator::TSqlGeneratorConfig());
        _Deserialize(p->sql.get(), sql_json.GetObject());
    }
}
//---------------------------------------------------------------------------------------
...
void TJsonSerializer::_Serialize(nsReflectionCodeGenerator::TTargetForCodeGeneration* p, Jobj& obj)
{
    ...
    Jobj implementation_obj = PUM::AddObject(obj, "implementation");
    _Serialize(&(p->implementation), implementation_obj);

    Jobj typeCustomizerMap_c0 = PUM::AddObject(obj, "typeCustomizerMap");
...
void TJsonSerializer::_Deserialize(nsReflectionCodeGenerator::TTargetForCodeGeneration* p, const Jobj& obj)
{
    ...
    auto implementation_json = POM::FindObject(obj, "implementation");
    _Deserialize(&(p->implementation), implementation_json);

    auto typeCustomizerMap_src0 = POM::FindObject(obj, "typeCustomizerMap");
...
{
    PUM::Value directories_c0(rapidjson::kArrayType);
    for ( int directories_i0 = 0; directories_i0 < p->directories.size(); directories_i0++ ) {
        ...
        PUM::PushBack(directories_c0, directories_e0);
    }
...
{
    auto directories_src0 = POM::FindArray(obj, "directories");
