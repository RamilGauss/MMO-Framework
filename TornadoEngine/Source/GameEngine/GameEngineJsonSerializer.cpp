...
void TGameEngineJsonSerializer::_Serialize( TAppConfig* p, Jobj& obj )
{
    ...
    Jarr modules_c0;
    for ( int modules_i0 = 0 ; modules_i0 < p->modules.size() ; modules_i0++ )
    {
...
void TGameEngineJsonSerializer::_Serialize( TConveyerConfig* p, Jobj& obj )
{
    ...
    for ( auto& appList_e0 : p->appList )
    {
        ...
        if ( appList_e0_ptr )
        {
            ...
            appList_c0.insert( { PUM::ConvertToString( appList_e0.first ), appList_e0_obj } );
        }
        else
            appList_c0.insert( { PUM::ConvertToString( appList_e0.first ), nullptr } );
    }
