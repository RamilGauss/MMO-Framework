...
//---------------------------------------------------------------------------
void MyClass::Serialize( TPushMaster* pPushMaster )
{
	pPushMaster->Push( b );
	pPushMaster->Push( uc );
	pPushMaster->Push( c );
	...
	pPushMaster->PushArraySer<OtherClass>( vSer );//std::vector<ISerializable>
	...
	pPushMaster->PushMap_Ser<unsigned char,OtherClass>(mUcSer); //std::map<unsigned char,OtherClass> mUcSer;//std::map<unsigned char,ISerializable>
	...
	pPushMaster->PushMap_Ser<char,OtherClass>(mCSer); //std::map<char,OtherClass> mSer;//std::map<char,ISerializable> 
	...
	pPushMaster->PushMap_Ser<unsigned short,OtherClass>(mUsSer);  //std::map<unsigned short,OtherClass> mUsSer;  //std::map<unsigned short,ISerializable> 
	...
	pPushMaster->PushMap_Ser<short,OtherClass>(mSSer);  //std::map<short,OtherClass> mSSer; //std::map<short,ISerializable>
	...
	pPushMaster->PushMap_Ser<unsigned int,OtherClass>(mUiSer); //std::map<unsigned int,OtherClass> mUiSer;//std::map<unsigned int,ISerializable>
	...
	pPushMaster->PushMap_Ser<int,OtherClass>(mISer);  //std::map<int,ISerializable> mISer;//std::map<int,ISerializable>
	...
	pPushMaster->PushMap_Ser<float,OtherClass>(mFSer);  //std::map<float,OtherClass> mFSer;//std::map<float,ISerializable>
	...
	pPushMaster->PushMap_Ser<double,OtherClass>(mDSer); //std::map<double,ISerializable> mDSer;//std::map<double,ISerializable>
	...
	pPushMaster->PushMapStrSer<OtherClass>(mStrSer);  //std::map<std::string,ISerializable> mStrSer;//std::map<std::string,ISerializable>
	pPushMaster->PushMapStrPtrSer(mStrPtrSer);        //std::map<std::string,ISerializable*> mStrPtrSer;//std::map<std::string,ISerializable*>

	pPushMaster->PushMapSer_<OtherClass,bool>( mSerB );         //std::map<int,bool> mIB;
	pPushMaster->PushMapSer_<OtherClass,unsigned char>(mSerUc); //std::map<int,unsigned char> mIUc;
	pPushMaster->PushMapSer_<OtherClass,char>(mSerC);           //std::map<int,char> mIC;
	pPushMaster->PushMapSer_<OtherClass,unsigned short>(mSerUs);//std::map<int,unsigned short> mIUs;
	pPushMaster->PushMapSer_<OtherClass,short>(mSerS);          //std::map<int,short> mIS;
	pPushMaster->PushMapSer_<OtherClass,unsigned int>(mSerUi);  //std::map<int,unsigned int> mIUi;
	pPushMaster->PushMapSer_<OtherClass,int>(mSerI);  					 //std::map<int,int> mII;
	pPushMaster->PushMapSer_<OtherClass,float>(mSerF);					 //std::map<int,float> mIF;
	pPushMaster->PushMapSer_<OtherClass,double>(mSerD); 				 //std::map<int,double> mID;
	pPushMaster->PushMapSerStr<OtherClass>(mSerStr);    				 //std::map<int,std::string> mIStr;
	pPushMaster->PushMapSerSer<OtherClass,OtherClass>(mSerSer); //std::map<int,ISerializable> mISer;//std::map<int,ISerializable>
	pPushMaster->PushMapSerPtrSer<OtherClass>(mSerPtrSer);      //std::map<int,ISerializable*> mIPtrSer;//std::map<int,ISerializable*>
}
//----------------------------------------------------------------------
void MyClass::Deserialize( TPopMaster* pPopMaster )
{
	pPopMaster->Pop( b );
	pPopMaster->Pop( uc );
	pPopMaster->Pop( c );
	...
	pPopMaster->PopPtrSer<OtherClass>( pSerializable );//ISerializable*
	...
	pPopMaster->PopVectorSer<OtherClass>( vSer ); 		 //std::vector<OtherClass> vSer;//std::vector<ISerializable>
	pPopMaster->PopArrayPtrSer<OtherClass>( vPtrSer );//std::vector<OtherClass*> vPtrSer;//std::vector<ISerializable*>
	...
	pPopMaster->PopListSer<OtherClass>( lSer );      //std::list<OtherClass> lSer;//std::list<ISerializable>
	pPopMaster->PopArrayPtrSer<OtherClass>( lPtrSer );//std::list<OtherClass*> lPtrSer;//std::list<ISerializable*>
	...
	pPopMaster->PopSetSer<OtherClass>( sSer ); //std::set<OtherClass> sSer;
	...
	pPopMaster->PopMap_Ser<unsigned char,OtherClass>( mUcSer );      //std::map<unsigned char,OtherClass> mUcSer;//std::map<unsigned char,ISerializable>
	pPopMaster->PopMap_PtrSer<unsigned char,OtherClass>( mUcPtrSer );//std::map<unsigned char,OtherClass*>mUcPtrSer;//std::map<unsigned char,ISerializable*>
	...
	pPopMaster->PopMap_Ser<char,OtherClass>( mCSer );      //std::map<char,OtherClass> mSer;//std::map<char,ISerializable> 
	pPopMaster->PopMap_PtrSer<char,OtherClass>( mCPtrSer );//std::map<char,OtherClass*> mCPtrSer;//std::map<char,ISerializable*>
	...
	pPopMaster->PopMap_Ser<unsigned short,OtherClass>( mUsSer );      //std::map<unsigned short,OtherClass> mUsSer;  //std::map<unsigned short,ISerializable> 
	pPopMaster->PopMap_PtrSer<unsigned short,OtherClass>( mUsPtrSer );//std::map<unsigned short,OtherClass*> mUsPtrSer; //std::map<unsigned short,ISerializable*>
	...
	pPopMaster->PopMap_Ser<short,OtherClass>( mSSer );      //std::map<short,OtherClass> mSSer; //std::map<short,ISerializable>
	pPopMaster->PopMap_PtrSer<short,OtherClass>( mSPtrSer );//std::map<short,OtherClass*> mSPtrSer;//std::map<short,ISerializable*>
	...
	pPopMaster->PopMap_Ser<unsigned int,OtherClass>( mUiSer );      //std::map<unsigned int,OtherClass> mUiSer;//std::map<unsigned int,ISerializable>
	pPopMaster->PopMap_PtrSer<unsigned int,OtherClass>( mUiPtrSer );//std::map<unsigned int,OtherClass*> mUiPtrSer;//std::map<unsigned int,ISerializable*>
	...
	pPopMaster->PopMap_Ser<int,OtherClass>( mISer );      //std::map<int,ISerializable> mISer;//std::map<int,ISerializable>
	pPopMaster->PopMap_PtrSer<int,OtherClass>( mIPtrSer );//std::map<int,ISerializable*> mIPtrSer;//std::map<int,ISerializable*>
	...
	pPopMaster->PopMap_Ser<float,OtherClass>( mFSer );      //std::map<float,OtherClass> mFSer;//std::map<float,ISerializable>
	pPopMaster->PopMap_PtrSer<float,OtherClass>( mFPtrSer );//std::map<float,OtherClass*> mFPtrSer;//std::map<float,ISerializable*>
	...
	pPopMaster->PopMap_Ser<double,OtherClass>( mDSer );      //std::map<double,ISerializable> mDSer;//std::map<double,ISerializable>
	pPopMaster->PopMap_PtrSer<double,OtherClass>( mDPtrSer );//std::map<double,ISerializable*> mDPtrSer;//std::map<double,ISerializable*>
	...
	pPopMaster->PopMapStrSer<OtherClass>( mStrSer );      //std::map<std::string,ISerializable> mStrSer;//std::map<std::string,ISerializable>
	pPopMaster->PopMapStrPtrSer<OtherClass>( mStrPtrSer );//std::map<std::string,ISerializable*> mStrPtrSer;//std::map<std::string,ISerializable*>

	pPopMaster->PopMapSer_<OtherClass,bool>( mSerB );           		  //std::map<std::string,bool> mStrB;
	pPopMaster->PopMapSer_<OtherClass,unsigned char>( mSerUc ); 		  //std::map<std::string,unsigned char> mStrUc;
	pPopMaster->PopMapSer_<OtherClass,char>( mSerC );           		  //std::map<std::string,char> mStrC;
	pPopMaster->PopMapSer_<OtherClass,unsigned short>( mSerUs );		  //std::map<std::string,unsigned short> mStrUS;
	pPopMaster->PopMapSer_<OtherClass,short>( mSerS );          		  //std::map<std::string,short> mStrS;
	pPopMaster->PopMapSer_<OtherClass,unsigned int>( mSerUi );  		  //std::map<std::string,unsigned int> mStrUi;
	pPopMaster->PopMapSer_<OtherClass,int>( mSerI );   							//std::map<std::string,int> mStrI;
	pPopMaster->PopMapSer_<OtherClass,float>( mSerF ); 							//std::map<std::string,float> mStrF;
	pPopMaster->PopMapSer_<OtherClass,double>( mSerD );							//std::map<std::string,double> mStrD;
	pPopMaster->PopMapSerStr<OtherClass>( mSerStr );                 //std::map<std::string,std::string> mStrStr;
	pPopMaster->PopMapSerSer<OtherClass,OtherClass>( mSerSer );      //std::map<std::string,ISerializable> mStrSer;//std::map<std::string,ISerializable>
	pPopMaster->PopMapSerPtrSer<OtherClass,OtherClass>( mSerPtrSer );//std::map<std::string,ISerializable*> mStrPtrSer;//std::map<std::string,ISerializable*>
}
//----------------------------------------------------------------------
