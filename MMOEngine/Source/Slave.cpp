...
void TSlave::EndRcm(IScenario* pSc)
{
  ...
  // дл€ всех контекстов назначить св€зь вниз
                                                              ...
                                                              unsigned int id_client)
{
  ...
  // надо проверить, вдруг клиент решил вз€ть не свой ключ и ключ совпал
  ...
  // сессии не должно быть, он ведь впервые соедин€етс€
...
void TSlave::NeedContextByClientForSlaveKeyRcm(unsigned int key, bool donor)
{
  ...
  else
  {
    ...
    // верх и низ сесси€ назначит сам сценарий
    mControlSc->mRcm->SetContext(&pC->mRcm);
  }
}
//-------------------------------------------------------------------------
...
{
  // дл€ подстраховки и там и там удалить
  TContainerContextSc* pC = mMngContextClientSlaveRecipient->FindContextByKey(key);
  if(pC)
    mMngContextClientSlaveRecipient->DeleteByKey(key);
  else
  {
    // если  лиент исхитрилс€ и вместо отсылки "CheckInfoRecipient, ready to disconnect"
