...
//------------------------------------------------------------------------------------------------
TTreeJoint::TLoadedJoint* IBaseObject::GetTree()
{
  return pLoadedTree;
}
//------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------
const nsMathTools::TMatrix16* IBaseObject::GetWorld()const 
{
  return &mWorld;
}
//------------------------------------------------------------------------------------------------
void IBaseObject::ChangeTreeMatrix(std::string& name,nsMathTools::TMatrix16* matrix, bool def )
{
  mTree.ChangeMatrix(name, matrix, def);
  NotifyUpdate(eTree);
}
//------------------------------------------------------------------------------------------------
void IBaseObject::RefreshVectorMatrixByTree()
{
  mTree.GetMatrix(&mVectorMatrix);
  NotifyUpdate(eVectorMatrix);
}
//------------------------------------------------------------------------------------------------
TTreeJoint* IBaseObject::GetReadyTree()
{
  return &mTree;
}
//------------------------------------------------------------------------------------------------
std::vector<nsMathTools::TMatrix16*>* IBaseObject::GetVectorMatrix()
{
  return &mVectorMatrix;
}
//------------------------------------------------------------------------------------------------
